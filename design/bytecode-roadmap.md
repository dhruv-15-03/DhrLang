...
This document outlines the staged plan for introducing a portable Intermediate Representation (IR) and bytecode execution backend for DhrLang. The initial goal is *correctness parity* with the AST interpreter while enabling future optimizations (constant folding, dead code elimination, inlining) without perturbing source semantics.
...
4. Pluggable Backend: CLI flag `--backend=ast|ir|bytecode` (default ast).
...
As of v1.1.3:
- IR and bytecode backends are implemented and selectable via `--backend=ir` and `--backend=bytecode`.
- Backend selection is authoritative (no AST fallback).
- Parity tests cover core semantics including arrays, calls, fields, and exceptions.
- The bytecode VM validates bytecode before executing and supports an untrusted mode (`dhrlang.bytecode.untrusted=true`) with conservative caps.

## Phase 1: Core IR Data Structures (Completed)
...
## Phase 2: Lowering Pipeline (Completed)
...
## Phase 3: Baseline IR Interpreter (Completed)
...
## Phase 4: Objects, Arrays & Exceptions (Completed)
...
## Phase 5: Optimization Hooks (Next)
...
## Phase 6: Emission & Tooling (Ongoing)
- `--emit-ir` prints IR (JSON) for debugging.
- `--emit-bc` writes `.dbc` bytecode for bytecode VM debugging.
- Future: optional IR/bytecode validators as standalone tooling and CFG visualization.
...
1. All existing green tests also pass with `--backend=ir` and `--backend=bytecode`.
2. Diagnostic error codes identical between backends for negative tests.
3. Bench harness remains deterministic and backend-comparable.
