...
- [ ] Update `version` in `build.gradle` to the target release version (currently `1.1.3`)
...
# Test version
java -jar build\libs\DhrLang-1.1.3.jar --version

# Test help
java -jar build\libs\DhrLang-1.1.3.jar --help

# Test a working program
java -jar build\libs\DhrLang-1.1.3.jar input\simple_working_test.dhr

# Test JSON diagnostics (should output clean JSON only)
java -jar build\libs\DhrLang-1.1.3.jar --json --time input\parser_error_test.dhr
```
...
2. Choose tag: `v1.1.3`
3. Release title: `DhrLang v1.1.3`
4. Description: Include highlights from `CHANGELOG.md`
5. Upload assets:
   - `build/libs/DhrLang-1.1.3.jar` (required)
   - Optional: Create a ZIP with `lib/DhrLang-1.1.3.jar` and `LICENSE`
...
### 📋 Ready for Release
- Version: `1.1.3`
- Artifact: `build/libs/DhrLang-1.1.3.jar` (approx ~4-6 MB with dependencies)
